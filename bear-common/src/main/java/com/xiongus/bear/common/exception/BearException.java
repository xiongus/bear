...
public class BearException extends Exception {
  ...
  @Serial private static final long serialVersionUID = 8094000382894904613L;
  ...
  public BearException(final int errCode, final String errMsg) {
    super(errMsg);
    this.errCode = errCode;
    this.errMsg = errMsg;
  }

  public BearException(final int errCode, final Throwable throwable) {
    super(throwable);
    this.errCode = errCode;
    this.setCauseThrowable(throwable);
  }

  public BearException(final int errCode, final String errMsg, final Throwable throwable) {
  ...
  /**
   * http client error code, ome exceptions that occurred when the use the Nacos RestTemplate and
   * Nacos AsyncRestTemplate.
   */
